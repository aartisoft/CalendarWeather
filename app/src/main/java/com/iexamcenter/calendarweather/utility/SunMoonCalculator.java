...
public class SunMoonCalculator {
    ...
    //https://github.com/M66B/BackPackTrackII/blob/master/app/src/main/java/eu/faircode/backpacktrack2/SunMoonCalculator.java
    ArrayList<fullmoon> fullmoon = new ArrayList<>();
    /**
     * Radians to degrees.
     */
    ...
    public SunMoonCalculator(int year, int month, int day, int h, int m, int s,
        ...
        // Log.e("SunMoon","SunMoonCalculator:year"+year+"month"+month+"day"+day+"h"+h+"m"+ m+"obsLon"+obsLon+"obsLat"+obsLat+"s"+s);
    ...
    public static Calendar getSunRiseDate(double jd, int Y, int M, int D) throws Exception {
        if (jd == -1) return null;

        int[] date = SunMoonCalculator.getDate(jd);


        // Log.e("calcla", "calcal:2:" + date[0] + "/" + date[1] + "/" + date[2] + " " + date[3] + ":" + date[4] + ":" + date[5] + " UT");
        String timeZoneStr = TimeZone.getDefault().getID();
        Calendar calendar = Calendar.getInstance(TimeZone.getTimeZone(timeZoneStr),Locale.ENGLISH);
        ...
        calendar.set(Y, M - 1, D, calendar.get(Calendar.HOUR_OF_DAY), calendar.get(Calendar.MINUTE), calendar.get(Calendar.SECOND));
        return calendar;
    }

    public static Calendar getDateAsDate(double jd) throws Exception {
        if (jd == -1) return null;
