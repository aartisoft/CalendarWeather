...
public class ChoghadiyaListAdapter extends RecyclerView.Adapter<ChoghadiyaListAdapter.ItemViewHolder> {
    Context mContext;
    ...
    Resources mRes;
    int daysInMonth = 0;
    String grahabhoga,arambha,sesha,pkey_sunset, pkey_sunrise, dayChoghadiya, nightChoghadiya, mLang, pkey_time_to, pkey_time_next, epkey_nakshatra_byakti, pkey_nakshatra_byakti;
    private String[] choghadiya_arr, choghadiya_night_arr;


    public ChoghadiyaListAdapter(Context context, HashMap<String, CoreDataHelper> myPanchangHashMap, HashMap<String, ArrayList<String>> monthFestivalmap, int monthIndex, int myear, int mday, int pagePos) {
        mContext = context;
        mPagePos = pagePos;
        ...
        if(!CalendarWeatherApp.isPanchangEng) {
            choghadiya_arr = mContext.getResources().getStringArray(R.array.l_arr_choghadiya_day);
            choghadiya_night_arr = mContext.getResources().getStringArray(R.array.l_arr_choghadiya_night);
            ...
            dayChoghadiya = mContext.getResources().getString(R.string.l_daychoghadiya);
            nightChoghadiya = mContext.getResources().getString(R.string.l_nightchoghadiya);
            ...
            grahabhoga = mContext.getResources().getString(R.string.l_choghadiya);
...
// restore original locale
    ...
    @Override
    public void onViewDetachedFromWindow(ChoghadiyaListAdapter.ItemViewHolder holder) {
    ...
    private void setPanchnang(int position, ItemViewHolder holder) {
        ...
        try {
            ...
            long diffDay = (diff) / 8;
            long diffNight = (24 * 60 * 60 * 1000 - diff) / 8;
            ...
            int[] planetIndexNightWise = {0, 2, 4, 6, 1, 3, 5};
            ...
            for (int i = 0; i < 16; i++) {
                ...
                if (i < 8) {
                    ...
                    text1 = choghadiya_arr[j];
                    if(j==0 || j==4 || j==6 ){
                        horaTxt1.setTextColor(Color.parseColor("#d50000"));
                    }else{
                        horaTxt1.setTextColor(Color.parseColor("#FFAB00"));
                    }
                    j++;
                } else {

                    hora.setTimeInMillis(sunsetCal.getTimeInMillis() + (i - 8) * diffNight);
                    horaStart = getFormattedDate(hora);
                    hora.setTimeInMillis(sunsetCal.getTimeInMillis() + ((i - 8 + 1) * diffNight));
                    horaEnd = getFormattedDate(hora);
                    text1 = choghadiya_night_arr[k];
                    if(k==3 || k==4 || k==6 ){
                        horaTxt1.setTextColor(Color.parseColor("#d50000"));
                    }else{
                        horaTxt1.setTextColor(Color.parseColor("#FFAB00"));
                ...
                if (i == 0) {
                    ...
                    horaTxt.setText(Html.fromHtml(dayChoghadiya));
                    ...
                    header.setVisibility(View.VISIBLE);
                    horaTxt.setVisibility(View.VISIBLE);
                    horaTxt.setText(Html.fromHtml(nightChoghadiya));
                    title1.setText(grahabhoga);
                    title2.setText(arambha);
                    title3.setText(sesha);
                ...
                //   String text3 = "<STRONG>" + choghadiya_arr[k] + "</STRONG>";
